...
DCOS：管理和调度数据中心资源的操作系统

![](http://img.blog.csdn.net/20161001083537034)
...
**1. 长时效服务：**永久在线的服务，比如说：淘宝网，支付宝等，任何时刻都可以访问

**2. 短时效服务：**比如说：调用一个接口下载一个文件，下载完就失效的。短时效服务一般是成批次的，所有短时效服务又被称为批处理

**DCOS图表分析**：

* **IaaS层：**

 1.资源池化：提供商的计算资源被集中起来为多个客户提供多租户模式的服务，不同的物理或虚拟资源按客户需求动态地分配和再分配。
 
 2.虚拟资源池：虚拟资源池是指把服务器，存储、网络都做成一个虚拟的资源池，应用软件需要的资源可以在资源池里抓取，这样能够提高企业的资源的利用率。
 
 3.DVS、SDN（软件定义网络）：在软件层面对物理机、虚拟机进行进一步的虚拟化操作
 
 4.SDN（Software Defined Networking，软件定义网络）技术能很好地契合数据中心网络在集中网络管理、灵活组网  多路径转发、虚拟机部署和智能迁移、虚拟多租户、IaaS等方面的需求，非常适合在数据中心网络中应用。因此，基于SDN的云计算数据中心网络是未来数据中心网络的发展趋势。

* **LDKLe Distribution Kernel（LDK）**

 1、计算、网络、存储
 
 2、masos：**控制计算资源、网络资源、存储资源，当需要这三个资源的时候，向masos申请。
 
 3、日志收集，监控竖着的原因：每个层次都要收集日志，都要监控
 
 4、镜像管理：可以利用创建的镜像运行一个容器或者发布它们供其他人使用。通过存储、检索、发布镜像，其他人可 以使用镜像管理的所有元素。
 
 5、服务管理：计算存储等都有服务，所以需要服务管理
 
 6、调度代理层：下面有三个框架，可以对上一层屏蔽下面的这三个框架的差异。即：**通用API，其他服务框架可以通过可插拔的方式轻松接入，不与具体框架做强绑定关联。**
 
 **Beehive-Default**（蜂巢）
 
 **Marathon**（masos公司开源的资源调度框架）：Marathon是一个成熟的，轻量级的，扩展性很强的Apache Mesos的容器编排框架，它主要用来调度和运行常驻服务（long-running service）
 
 **Kubernetes**（谷歌公司开源的调度器，基于容器来分配计算资源、网络资源、存储资源）：是自动化容器操作的开源平台，这些操作包括部署，调度和节点集群间扩展。Kubernetes不仅仅支持Docker，还支持Rocket，这是另一种容器技术。
 
 7、Batch-job scheduler：批处理调度器（短时效调度）
 
 8、Task scheduler：任务调度器（长时效调度）
 
* **并行计算**

 1、hdfs：Hadoop分布式文件系统(HDFS)
 
 2、Batch computer：批处理。比如：将数据堆一个星期，开很多机器去跑这一批数据，去分析。
 
 3、streaming computer：流处理。一种被设计来处理无穷数据集的数据处理系统引擎。实时处理数据
 
* **微服务体系**

什么是大数据：

3V：产生数据速度快、数据规模大、数据种类多

为什么用DCOS呢：协调资源

**8.2 电商云服务治理框架**

![](http://img.blog.csdn.net/20161001083601425)
...
参考:https://blog.csdn.net/suifeng3051/article/details/53992560

一个大型的系统，会有多个应用，应用之间有共同的业务，而且应用之间还存在调用关系。
因此，为了使业务得到很好的复用，模块更加容易拓展和维护，我们希望

1. **业务与应用分离，某个业务不再属于一个应用，而是作为一个独立的服务单独进行维护**。

2. **应用本身由一个个模块化的服务组件组合而成。**

应用与服务的关系如下图所示：↓

![](https://img-blog.csdn.net/20170103142540155?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvc3VpZmVuZzMwNTE=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
服务化主要有如下**特点**：
* **应用按业务拆分成服务：即应用可以有多个服务**
* **各个服务均可独立部署**
* **服务可被多个应用共享：即服务可以被多个应用使用**
* **服务之间可以通信**

## 2. 服务化实现方式
### 要想服务化，**服务治理**是关键 使用带有服务治理功能的RPC框架。

**如果要实现服务化的话，最常用的方式就是利用RPC框架。因为服务组件一般分布在不同的服务器上，所以要实现服务化需要解决的第一个问题就是RPC远程服务调用**。

**RPC（Remote Procedure Call）：远程过程调用。**它是一种通过网络从远程计算机程序上请求服务，而不需要了解底层网络技术的协议。可以理解为：接口调用的另一种方式，只不过这个接口的背后是函数调用。

### dubbo服务治理框架
![](https://img-blog.csdn.net/20170103143859427?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvc3VpZmVuZzMwNTE=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
## 3. 微服务
...
**把服务再拆分成一个个API，API是一个完整的功能。然后把API扔到一个“云上”，用户就可以到“云上”获取所有API的服务，这个“云”保证能提供好的服务。**
